...
async function superResolutionAndSave(
  ...
  const apiConfig = await u.getPromptAi("storyboardImage", accountId);
  ...
  const ossPath = buildAccountPath(accountId, `/${projectId}/chat/${v4()}.jpg`);
...
export default router.post(
  ...
  async (req, res) => {
    ...
    const scriptData = await u.db("t_script").where("id", scriptId).select("content", "projectId").first();
    if (!scriptData) return res.status(500).send(error("剧本不存在"));
    if (Number(scriptData.projectId) !== Number(projectId)) return res.status(403).send(error("剧本不属于当前项目"));
    ...
    const processSegment = async (segment: { cells: { id: string; src: string }[] }) => {
      ...
      const cellsWithSuperscore = await Promise.all(
        segment.cells.map(async (cell) => {
          const { ossPath } = await superResolutionAndSave(cell.src, projectId, projectData.videoRatio!, accountId);
          return {
            id: cell.id,
            projectId,
            scriptId,
