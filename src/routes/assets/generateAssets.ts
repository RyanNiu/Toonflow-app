...
export default router.post(
  ...
  async (req, res) => {
    ...
    const asset = await u.db("t_assets").where({ id, projectId }).first();
    ...
    **分镜设定：**
    ...
    const apiConfig = await u.getPromptAi("assetsImage", accountId);
    ...
    try {
      ...
      if (type == "role") {
        insertType = "角色";
         imagePath = buildAccountPath(accountId, `/${projectId}/role/${uuidv4()}.jpg`);
      }
      if (type == "scene") {
        insertType = "场景";
         imagePath = buildAccountPath(accountId, `/${projectId}/scene/${uuidv4()}.jpg`);
      }
      if (type == "props") {
        insertType = "道具";
         imagePath = buildAccountPath(accountId, `/${projectId}/props/${uuidv4()}.jpg`);
      }
      if (type == "storyboard") {
        insertType = "分镜";
         imagePath = buildAccountPath(accountId, `/${projectId}/storyboard/${uuidv4()}.jpg`);
      }
