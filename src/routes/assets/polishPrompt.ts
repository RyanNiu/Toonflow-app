...
function normalizeChapterRange(range?: number[] | number | null): number[] {
  if (Array.isArray(range)) {
    return range.filter((item) => typeof item === "number" && Number.isFinite(item));
  }
  if (typeof range === "number" && Number.isFinite(range)) return [range];
  return [];
}
//润色提示词
export default router.post(
  ...
  async (req, res) => {
    ...
    const projectAccess = await getProjectForAccount(accountId, projectId);
    if (!projectAccess) return res.status(404).send({ message: "项目不存在" });
    ...
    const promptsList = await u
      ...
      .where("code", "in", ["role-polish", "scene-polish", "storyboard-polish", "tool-polish"]);
    const apiConfigData = await u.getPromptAi("assetsPrompt", accountId);
    ...
    if (type == "role") {
      ...
      if (chapterRange.length > 0) {
        ...
        results = mergeNovelText(novelData);
      }
      systemPrompt = role;
      userPrompt = `
      请根据以下参数生成角色标准四视图提示词：
    ...
    async function generatePrompt() {
      ...
      const text = (result as { text?: string; prompt?: string } | undefined)?.text ??
        (result as { prompt?: string } | undefined)?.prompt ??
        "";
