  ...
  async (req, res) => {
    ...
    if (fileUrl.length) {
      ...
      if (match && match.length >= 2) {
        const imagePath = buildAccountPath(accountId, `/${projectId}/assets/${uuidv4()}.jpg`);
    ...
    const savePath = buildAccountPath(accountId, `/${projectId}/video/${uuidv4()}.mp4`);
 ...
 */
async function sharpProcessingImage(imageList: string[], projectId: number, accountId: number): Promise<string> {
