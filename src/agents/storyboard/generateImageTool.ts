...
// 使用 AI 过滤与分镜相关的资产
async function filterRelevantAssets(
  prompts: string[],
  ...
  const apiConfig = await u.getPromptAi("storyboardAgent", undefined, projectId);
...
export default async (cells: { prompt: string }[], scriptId: number, projectId: number) => {
  ...
  // 使用 AI 过滤相关资产
  const filteredImages = await filterRelevantAssets(cellPrompts, resources, allImages, projectId);
  ...
  console.log("====开始处理引用的图片资产（压缩等）====...");
  const processedImages = await processImages(filteredImages);
  console.log(`====处理图片完成，最终提交给 AI 的图片数量: ${processedImages.length} ====`);
  const apiConfig = await u.getPromptAi("storyboardImage", undefined, projectId);

  console.log("====准备调用大模型统一生图入口 u.ai.image... ====");
  const aiConfigAny = apiConfig as any;
  console.log(`====[u.ai.image] 参数：Model: ${aiConfigAny?.model || "未知"} | 厂家: ${aiConfigAny?.manufacturer || "未知"}`);
  console.log(`====[u.ai.image] 第一张图片大小约: ${processedImages.length > 0 ? Math.round(processedImages[0].length / 1024) : 0} KB`);
  ...
  try {
    ...
    console.log("====大模型统一生图接口 u.ai.image 调用成功！====");
  } catch (err: any) {
    console.error("====[ERROR] 大模型统一生图接口 u.ai.image 抛出异常:====", err?.message || err);
    throw err;
  }
