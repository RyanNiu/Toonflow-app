...
async function generateGridPrompt(options: GridPromptOptions): Promise<GridPromptResult> {
  ...
  const project = await u.db("t_project").where({ id: options.projectId }).select("accountId").first();
  const accountId = Number(project?.accountId) || 1;
  const promptsData = await u
    .db("t_prompts")
    .where("accountId", accountId)
    .where("code", "generateImagePrompts")
    .first();
  const promptAiConfig = await u.getPromptAi("storyboardAgent", accountId, options.projectId);
