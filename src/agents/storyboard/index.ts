...
export default class Storyboard {
  ...
  private async getAccountId(): Promise<number> {
    ...
    const project = await u.db("t_project").where({ id: this.projectId }).select("accountId").first();
...
${task}
  ...
  private async invokeSubAgent(agentType: AgentType, task: string): Promise<string> {
    ...
    const promptsList = await u
      ...
      .where("code", "in", ["storyboard-segment", "storyboard-shot"]);
