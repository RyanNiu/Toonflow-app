...
// 只包含 t_setting_account 表里实际存在的字段
const modelFields = {
  image: "imageModel",
...
// 主方法
export default async function getConfig<T extends ModelType | "video">(
  type: T,
  manufacturer?: string,
  accountId?: number,
  ...
  const data: Record<string, any> | undefined = await u
    .db("t_setting_account")
    .where({ accountId: accountId ?? 1 })
    .select([modelName])
    .first();
