...
const buildOptions = async (input: AIInput<any>, config: AIConfig = {}) => {
  ...
  const chatModelManufacturer = ["volcengine", "other", "openai", "modelScope", "grsai", "zzgf"];
...
ai.invoke = async (input: AIInput<any>, config: AIConfig) => {
  ...
  let result: GenerateTextResult<Record<string, Tool>, never>;
  try {
    result = await generateText(options.config);
  } catch (err: any) {
    const message = u.error(err).message;
    console.error("[Text AI] Error", {
      message,
      response: err?.response ? JSON.stringify(err.response).slice(0, 800) : undefined,
      cause: err?.cause ? JSON.stringify(err.cause).slice(0, 800) : undefined,
    });
    throw err;
  }
