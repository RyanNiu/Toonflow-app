 ...
 */
export async function generateScript(episode: Episode, novelData: string, accountId?: number): Promise<string> {
...
${novelData}`;
  ...
  const prompts = await u
    ...
    .where("code", "script")
    .first();
  const promptConfig = await u.getPromptAi("generateScript", accountId);
