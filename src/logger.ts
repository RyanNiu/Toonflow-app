...
// ⚠️ 惰性求值：不能在模块顶层立即调用 app.getPath()，
//    否则在 Windows 上 Electron app 未就绪时会崩溃。
let _logDir: string | null = null;
let _logFile: string | null = null;
function getLogDirLazy(): string {
  if (!_logDir) _logDir = getLogDir();
  return _logDir;
}
function getLogFileLazy(): string {
  if (!_logFile) _logFile = path.join(getLogDirLazy(), "app.log");
  return _logFile;
}
...
class Logger {
  ...
  init(): this {
    const LOG_DIR = getLogDirLazy();
    const LOG_FILE = getLogFileLazy();
