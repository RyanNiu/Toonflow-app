...
export default async (knex: Knex): Promise<void> => {
  ...
  const resolveUploadPath = (relPath: string) => {
    const trimmed = relPath.replace(/^[/\\]+/, "");
    return path.join(uploadsRoot, trimmed);
  };

  const moveIfNeeded = async (fromPath?: string | null, toPath?: string | null) => {
    if (!fromPath || !toPath || fromPath === toPath) return;
    if (fromPath.startsWith("http://") || fromPath.startsWith("https://")) return;
    const fromAbs = resolveUploadPath(fromPath);
    const toAbs = resolveUploadPath(toPath);
    if (fromAbs === toAbs) return;
    if (!fs.existsSync(fromAbs)) return;
    if (!fs.existsSync(path.dirname(toAbs))) {
  ...
  await addColumn("t_user", "is_admin", "integer");
  await addColumn("t_user", "deleted_at", "integer");
  await addColumn("t_project", "accountId", "integer");
  await addColumn("t_config", "accountId", "integer");
  await addColumn("t_prompts", "accountId", "integer");
  await addColumn("t_aiModelMap", "accountId", "integer");
  await addColumn("t_taskList", "projectId", "integer");
  await addColumn("t_taskList", "accountId", "integer");

  // 创建新设置表
  if (!(await knex.schema.hasTable("t_setting_system"))) {
    await knex.schema.createTable("t_setting_system", (table) => {
  ...
  if (!(await knex.schema.hasTable("t_setting_account"))) {
    await knex.schema.createTable("t_setting_account", (table) => {
      table.integer("id").notNullable();
  ...
  // 初始化账号字段
  if (await knex.schema.hasColumn("t_user", "is_admin")) {
    await knex("t_user").whereNull("is_admin").update({ is_admin: 0 });
    await knex("t_user").where({ id: 1 }).update({ is_admin: 1 });
  }
  if (await knex.schema.hasColumn("t_project", "accountId")) {
    if (await knex.schema.hasColumn("t_project", "userId")) {
      await knex("t_project").whereNull("accountId").update({ accountId: knex.raw("userId") });
    }
    await knex("t_project").whereNull("accountId").update({ accountId: 1 });
  }
  if (await knex.schema.hasColumn("t_config", "accountId")) {
    if (await knex.schema.hasColumn("t_config", "userId")) {
      await knex("t_config").whereNull("accountId").update({ accountId: knex.raw("userId") });
    }
    await knex("t_config").whereNull("accountId").update({ accountId: 1 });
  }
  if (await knex.schema.hasColumn("t_prompts", "accountId")) {
    await knex("t_prompts").whereNull("accountId").update({ accountId: 1 });
  }
  if (await knex.schema.hasColumn("t_aiModelMap", "accountId")) {
    await knex("t_aiModelMap").whereNull("accountId").update({ accountId: 1 });
  }

  if (await knex.schema.hasColumn("t_taskList", "accountId")) {
    await knex("t_taskList").whereNull("accountId").update({ accountId: 1 });
  }

  // 迁移旧 t_setting 数据到新表
  if (await knex.schema.hasTable("t_setting")) {
    const legacy = await knex("t_setting").where({ id: 1 }).first();
    const systemExists = await knex("t_setting_system").where({ id: 1 }).first();
    if (!systemExists) {
      await knex("t_setting_system").insert({
        id: 1,
        key: "requestConfig",
        value: "{}",
        createTime: Date.now(),
    ...
    const accountExists = await knex("t_setting_account").where({ accountId: 1 }).first();
    if (!accountExists) {
      await knex("t_setting_account").insert({
        id: 1,
        accountId: 1,
        imageModel: legacy?.imageModel ?? "{}",
        languageModel: legacy?.languageModel ?? "{}",
      });
    }
  }

  if (await knex.schema.hasTable("t_setting")) {
    await knex.schema.dropTable("t_setting");
  }

  // 补齐账号设置
  if (await knex.schema.hasTable("t_setting_account") && (await knex.schema.hasTable("t_user"))) {
    const users = await knex("t_user").select("id");
    const existing = await knex("t_setting_account").select("accountId");
    const existingSet = new Set(existing.map((i) => i.accountId));
    const template = await knex("t_setting_account").where({ accountId: 1 }).first();
    const maxIdRow: any = await knex("t_setting_account").max("id as maxId").first();
  ...
  // 补齐账号默认配置数据
  if ((await knex.schema.hasTable("t_user")) && (await knex.schema.hasTable("t_config"))) {
    const users = await knex("t_user").whereNull("deleted_at").select("id");
    const templateConfigs = await knex("t_config").where({ accountId: 1 }).select("*");
    const templatePrompts = await knex("t_prompts").where({ accountId: 1 }).select("*");
    const templateAiModels = await knex("t_aiModelMap").where({ accountId: 1 }).select("*");

    let configIdCursor = ((await knex("t_config").max("id as maxId").first()) as any)?.maxId ?? 0;
    let promptIdCursor = ((await knex("t_prompts").max("id as maxId").first()) as any)?.maxId ?? 0;
    let aiModelIdCursor = ((await knex("t_aiModelMap").max("id as maxId").first()) as any)?.maxId ?? 0;

    for (const user of users) {
      const accountId = Number(user.id);
      if (!accountId || accountId === 1) continue;

      const configCount: any = await knex("t_config").where({ accountId }).count("id as total").first();
      if (Number(configCount?.total ?? 0) === 0 && templateConfigs.length) {
        const inserts = templateConfigs.map((item: any) => ({
          ...item,
          id: ++configIdCursor,
          accountId,
        }));
        await knex("t_config").insert(inserts);
      }

      if (templatePrompts.length) {
        const existingPromptCodes = await knex("t_prompts").where({ accountId }).select("code");
        const existingSet = new Set(existingPromptCodes.map((p) => p.code));
        const missingPrompts = templatePrompts.filter((p: any) => !existingSet.has(p.code));
        if (missingPrompts.length) {
          const inserts = missingPrompts.map((item: any) => ({
            ...item,
            id: ++promptIdCursor,
            accountId,
          }));
          await knex("t_prompts").insert(inserts).onConflict(["code", "accountId"]).ignore();
        }
      }

      if (templateAiModels.length) {
        const existingKeys = await knex("t_aiModelMap").where({ accountId }).select("key");
        const existingKeySet = new Set(existingKeys.map((k) => k.key));
        const missing = templateAiModels.filter((m: any) => !existingKeySet.has(m.key));
        if (missing.length) {
          const inserts = missing.map((item: any) => ({
            ...item,
            id: ++aiModelIdCursor,
            accountId,
            configId: null,
          }));
          await knex("t_aiModelMap").insert(inserts);
        }
      }
    }
  }
  ...
  // 清理已废弃的 userId 字段
  await dropColumn("t_project", "userId");
  await dropColumn("t_config", "userId");

  if (await knex.schema.hasTable("t_prompts")) {
    await knex.schema.raw("DROP INDEX IF EXISTS t_prompts_code_unique");
    await knex.schema.raw("DROP INDEX IF EXISTS t_prompts_code");
    await knex.schema.raw("CREATE UNIQUE INDEX IF NOT EXISTS idx_prompts_code_account ON t_prompts(code, accountId)");
  }

  // 明文密码迁移为哈希
  if (await knex.schema.hasColumn("t_user", "password")) {
    const users = await knex("t_user").select("id", "password");
    for (const user of users) {
      ...
      if (!isHashed && password) {
        ...
        await knex("t_user").where({ id: user.id }).update({ password: hashed });
      }
    }
  }

  const withAccountPrefix = (filePath: string | null | undefined, accountId: number) => {
    if (!filePath) return filePath;
    if (filePath.startsWith("http://") || filePath.startsWith("https://")) return filePath;
    if (filePath.startsWith(`/${accountId}/`)) return filePath;
    if (filePath.startsWith("/")) return `/${accountId}${filePath}`;
    return `/${accountId}/${filePath}`;
  };

  // 旧文件路径补齐 accountId 前缀
  if (await knex.schema.hasColumn("t_project", "accountId")) {
    const projects = await knex("t_project").select("id", "accountId");
    for (const project of projects) {
      const projectId = Number(project.id);
      const accountId = Number(project.accountId) || 1;
      if (!projectId) continue;

      const assets = await knex("t_assets").where({ projectId }).select("id", "filePath");
      for (const asset of assets) {
        const nextPath = withAccountPrefix(asset.filePath, accountId);
        if (nextPath !== asset.filePath) {
          await moveIfNeeded(asset.filePath, nextPath);
          await knex("t_assets").where({ id: asset.id }).update({ filePath: nextPath });
        }
      }

      const images = await knex("t_image").where({ projectId }).select("id", "filePath");
      for (const image of images) {
        const nextPath = withAccountPrefix(image.filePath, accountId);
        if (nextPath !== image.filePath) {
          await moveIfNeeded(image.filePath, nextPath);
          await knex("t_image").where({ id: image.id }).update({ filePath: nextPath });
        }
      }

      const scripts = await knex("t_script").where({ projectId }).select("id");
      const scriptIds = scripts.map((s) => s.id).filter(Boolean);
      if (scriptIds.length) {
        const videos = await knex("t_video").whereIn("scriptId", scriptIds).select("id", "filePath", "firstFrame", "storyboardImgs");
        for (const video of videos) {
          const nextFilePath = withAccountPrefix(video.filePath, accountId);
          const nextFirstFrame = withAccountPrefix(video.firstFrame, accountId);
          let nextStoryboardImgs = video.storyboardImgs;
          if (video.storyboardImgs) {
            try {
              const parsed = Array.isArray(video.storyboardImgs)
                ? video.storyboardImgs
                : JSON.parse(video.storyboardImgs);
              const mapped = parsed.map((p: string) => withAccountPrefix(p, accountId));
              nextStoryboardImgs = JSON.stringify(mapped);
            } catch {
              nextStoryboardImgs = video.storyboardImgs;
            }
          }
          if (
            nextFilePath !== video.filePath ||
            nextFirstFrame !== video.firstFrame ||
            nextStoryboardImgs !== video.storyboardImgs
          ) {
            await moveIfNeeded(video.filePath, nextFilePath);
            await moveIfNeeded(video.firstFrame, nextFirstFrame);
            if (video.storyboardImgs) {
              try {
                const parsed = Array.isArray(video.storyboardImgs)
                  ? video.storyboardImgs
                ...
                for (const item of parsed) {
                  const nextItem = withAccountPrefix(item, accountId);
                  await moveIfNeeded(item, nextItem);
                }
              } catch {
                // ignore
              }
            }
            await knex("t_video").where({ id: video.id }).update({
              filePath: nextFilePath,
              firstFrame: nextFirstFrame,
              storyboardImgs: nextStoryboardImgs,
            });
          }
        }
      }

      const videoConfigs = await knex("t_videoConfig").where({ projectId }).select("id", "startFrame", "endFrame", "images");
      for (const cfg of videoConfigs) {
        let nextStart = cfg.startFrame;
        let nextEnd = cfg.endFrame;
        let nextImages = cfg.images;

        if (cfg.startFrame) {
          try {
            const parsed = JSON.parse(cfg.startFrame);
            if (parsed?.filePath) parsed.filePath = withAccountPrefix(parsed.filePath, accountId);
            nextStart = JSON.stringify(parsed);
          } catch {
            nextStart = cfg.startFrame;
          }
        }

        if (cfg.endFrame) {
          try {
            const parsed = JSON.parse(cfg.endFrame);
            if (parsed?.filePath) parsed.filePath = withAccountPrefix(parsed.filePath, accountId);
            nextEnd = JSON.stringify(parsed);
          } catch {
            nextEnd = cfg.endFrame;
          }
        }

        if (cfg.images) {
          try {
            const parsed = JSON.parse(cfg.images);
            const mapped = Array.isArray(parsed)
              ? parsed.map((img: any) => ({
                  ...img,
                  filePath: img?.filePath ? withAccountPrefix(img.filePath, accountId) : img?.filePath,
                }))
              : parsed;
            nextImages = JSON.stringify(mapped);
          } catch {
            nextImages = cfg.images;
          }
        }

        if (nextStart !== cfg.startFrame || nextEnd !== cfg.endFrame || nextImages !== cfg.images) {
          if (cfg.startFrame) {
            try {
              const parsed = JSON.parse(cfg.startFrame);
              if (parsed?.filePath) await moveIfNeeded(parsed.filePath, withAccountPrefix(parsed.filePath, accountId));
            } catch {
              // ignore
            }
          }
            ...
            try {
              ...
              if (Array.isArray(parsed)) {
                for (const img of parsed) {
                  if (img?.filePath) await moveIfNeeded(img.filePath, withAccountPrefix(img.filePath, accountId));
                }
              }
            } catch {
              // ignore
          ...
          await knex("t_videoConfig").where({ id: cfg.id }).update({
  ...
  const existItems = await knex("t_aiModelMap").where("accountId", 1).whereIn("key", keys).select("key");
